...
function runApplescript(strOrPath, args, callback) {
  ...
  node.interpreter = spawn(exports.osascript, args);
  ...
  interpreter.on('close', function(code) {
    ...
    if (code) {
      ...
      // Parse the output only in case of success
      try {
        result = parse(interpreter.stdout.body);
      } catch (e) {
        // If there is a parse error, set the err value.
        err = e;
      }
    }
    if (callback) {
